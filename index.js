...
async function starts() {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			const ownerNumber = ["62895330379186@s.whatsapp.net"] // replace this with your number
			...
			const isBirdha = birdhaNumber.includes(sender)
                        ...
                        const NomerOwner = '62895330379186@s.whatsapp.net'
                        /******ApiKey Input******/
			...
			case 'creator':
  					...
  					client.sendMessage(from, 'Nih nomor ownerku kak, Jika ingin bertanya tentangku atau lainnya ^_^"',MessageType.text, { quoted: mek} )
  					break
			case 'hidetag':
  					client.updatePresence(from, Presence.composing) 
  					if (!isUser) return reply(mess.only.daftarB)
			...
			case 'snow':
					...
					var gbl7 = gh.split("|")[0];
					var gbl8 = gh.split("|")[1];
					if (args.length < 1) return reply(`Kirim perintah ${prefix}snowwrite teks1|teks2, contoh ${prefix}snowwrite Aine|cool`)
                                        if (!isUser) return reply(mess.only.daftarB)
					reply(mess.wait)
			...
			case 'marvellogo':
					...
					if (args.length < 1) return reply(`Kirim perintah ${prefix}marvellogo teks, contoh ${prefix}marvellogo Aine cool`)
                                        if (!isUser) return reply(mess.only.daftarB)
					reply(mess.wait)
			...
			case 'artinama':
					client.updatePresence(from, Presence.composing) 
					if (!isUser) return reply(mess.only.daftarB)
					data = await fetchJson(`https://mnazria.herokuapp.com/api/arti?nama=${body.slice(10)}`)
					reply(data.result)
					break
			case 'infonomor':
					client.updatePresence(from, Presence.composing) 
					if (!isUser) return reply(mess.only.daftarB)
					if (args.length < 1) return reply(`Masukan Nomor\nContoh : ${prefix}infonomor 0895330379186`)
			...
			case 'semoji':
				...
				case 'tulis':
					...
					teks = body.slice(7)
					reply(mess.wait)
					anu = await fetchJson(`https://st4rz.herokuapp.com/api/nulis?text=haloh`, {method: 'get'})
					if (anu.error) return reply(anu.error)
				...
				case 'tts':
					...
					: gtts.save(ranm, dtt, function() {
						exec(`ffmpeg -i ${ranm} -ar 48000 -vn -c:a libopus ${rano}`, (err) => {
							...
							if (err) return reply('Yaahh.. gagal kak :/')
				...
				case 'block':
					...
					client.sendMessage(from, `*Perintah Diterima, Memblockir* ${body.slice(8)}@c.us`, text)
					break
			       case 'block1':
					client.updatePresence(from, Presence.composing) 
					if (!isGroup) return reply(mess.only.group)
				...
				case 'bcgc':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of groupMembers) {
							client.sendMessage(_.jid, buff, image, {caption: `*「 PESAN BROADCAST GROUP 」*\n*Group* : ${groupName}\n\n${body.slice(6)}`})
						}
						reply('')
					} else {
						for (let _ of groupMembers) {
							sendMess(_.jid, `*「 PESAN BROADCAST GROUP 」*\n*Group* : ${groupName}\n\n${body.slice(6)}`)
						}
				...
				case 'kapankah':
					...
					const kapan =['1 Hari lagi','2 hari lagi','3 hari lagi','4 hari lagi','5 hari lagi','6 hari lagi','1 minggu lagi','2 minggu lagi','3 minggu lagi','1 bulan lagi','2 bulan lagi','3 hari lagi','4 bulan lagi','5 bulan lagi','6 hari lagi','7 bulan lagi','8 bulan lagi','9 hari lagi','10 bulan lagi','11 bulan lagi','1 tahun lagi','2 tahun lagi','3 tahun lagi','4 tahun lagi','Tidak akan','Yakin bakal terjadi ?','Aku meragukan nya','Lusa','Akhir bulan depan','Awal bulan depan','Tahun depan','Bulan depan','Sebentar lagi']
				...
				case 'logowolf2':
					...
					var gbl1 = gh.split("|")[0];
					var gbl2 = gh.split("|")[1];
					if (args.length < 1) return reply('Teksnya mana um\nContoh: ${prefix}phlogo |Aine|Cools')
                                        if (!isUser) return reply(mess.only.daftarB)
					reply(mess.wait)
				...
				case 'leveling':
					...
					break
                                ...
                                case 'lionlogo':
                                        ...
                                        var teks1 = gh.split("|")[0];
                                        var teks2 = gh.split("|")[1];
                                        if (args.length < 1) return reply('teksnya mana um\nContoh: ${prefix}lionlogo Aine|Cool')
                                        if (!isUser) return reply(mess.only.daftarB)
                                        reply(mess.wait)
					...
					break
				       ...
				       break
                                ...
                                case 'undergocean':
                                        var gh = body.slice(12)
                                        var teks1 = gh.split("|")[0];
                                        if (args.length < 1) return reply('teksnya mana um\nContoh: ${prefix}undergocean Aine')
                                        if (!isUser) return reply(mess.only.daftarB)
                                        reply(mess.wait)
                                ...
                                case 'woodenboards':
                                        ...
                                        var teks1 = gh.split("|")[0];
                                        if (args.length < 1) return reply('teksnya mana um\nContoh: ${prefix}woodenboards Aine')
                                        if (!isUser) return reply(mess.only.daftarB)
                                        reply(mess.wait)
                                ...
                                case 'metalictglow':
                                        ...
                                        var teks1 = gh.split("|")[0];
                                        if (args.length < 1) return reply('teksnya mana um\nContoh: ${prefix}metalictglow Aine')
                                        if (!isUser) return reply(mess.only.daftarB)
                                        reply(mess.wait)
